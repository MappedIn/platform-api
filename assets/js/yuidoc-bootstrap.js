...
$(function() {
	...
	function setupActiveSidebarLink() {
		...
		if (pathname) {
			...
			var path = decodeURIComponent(
				pathArr.slice(pathArr.length - 2, pathArr.length).join('/')
			); // modules/My Module.html
	...
	function setupHeaderAnchors() {
		...
		$('h1, h2, h3, h4, h5, h6', '.module-description, .class-description').each(
			function() {
				var $element = $(this);
				...
				$element.prepend(
					'<a class="fa fa-link toc-anchor" href="#' + id + '"></a>'
				);
			}
		);
	}
		...
		if (e.ctrlKey) {
			...
			if (e.keyCode === 40) {
				...
				if ($('#api-tabview-filter input').is(':focus')) {
					...
					$this.find('.tab-content .tab-pane.active li:last a').is(':focus')
				) {
					...
					nextIndex =
						$this
							...
							.index() + 1;
					$this
						.find('.tab-content .tab-pane.active li:eq(' + nextIndex + ') a')
						.focus();
				}
				...
				if ($('#api-tabview-filter input').is(':focus')) {
					...
					$this.find('.tab-content .tab-pane.active li:first a').is(':focus')
				) {
