...
The MappedIn API is a interface to the MappedIn backend for venue management and indoor wayfinding. It is a JSON-only API inspired by RESTful principles. The API makes uses HTTP verbs to express intent and status codes to indicate outcome.
...
Removes a single object from the job collection
...
|`direction`|String|Exit direction.|
|`directionsName`|String|Alternate name or short description that is substituted for the location name turn-by-turn directions when the location is being used as a landmark (e.g. "turn left at the bank" vs. "turn left at Cartesian Credit Union").|
